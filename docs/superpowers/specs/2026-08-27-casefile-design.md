...
casefile is a locally-run OSINT pivot console. Install it, run it, and it opens a browser
on itself. From there it is a browser tool: one input box takes any identifier, detects
what it could be, and fans out across hundreds of public sources, with deep links rendered
instantly and live results streaming in as they land.

The terminal is a launcher, not the interface. `casefile` with no arguments is the whole
usage story; the query subcommands exist for scripting and are secondary.
...
- Install, run, use it in a browser. Three steps, no configuration, no reading first.
...
| Invocation | Effect |
|---|---|
| `casefile` | Launch the app, print the URL, open a browser on it. **The intended usage.** |
| `casefile --no-browser` | Same, without opening a browser. Headless and SSH sessions. |
...
- **No telemetry, ever, and nothing leaves the machine.** Two things reach disk and they are
  ...
  is a notebook. Amended 2026-08-29, see the cases design.
